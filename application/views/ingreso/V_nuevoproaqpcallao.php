...
<div class="section container">
	<div class="row">
		...
		<div class="input-field col s12 m6 l3" style="margin-top:-7px;" id="idtipodocumento">
			...
			<input class="ancho" id="tipodocumento" type="text" value="" readonly disabled>
			<br>
	...
	<div class="row">
		<div class="col s8">
			...
			<div class="input-field col s12" >
				<input type="hidden" id="idupdate" name="idupdate" value="">
					...
					<input class="ancho" id="empresa" type="text" value="" disabled readonly> <!-- ondblclick="EditarEmpresa()"-->
			...
			<div class="input-field col s6"  >
				<input id="pase" type="number" value="" name="pase" disabled readonly >
                <label class="active" for="pase">Pase</label> 
			</div>
			
			<div class="input-field col s6"  >
				<input id="temperatura" type="text" value="" name="temperatura" disabled readonly >
		...
		<div id="divtipo_unidad2" style="display: none;">
			...
			<!--
			<div class="input-field col s12 ">
			<input type="text" id="autocomplete-input2" class="autocomplete2" autocomplete="off" disabled readonly  >
			<input type="hidden" id="cliente" name="cliente" >
			<label style="color:#0000006b" id="labelempresa" >Empresa Visitada</label>
			</div>
			-->
			
			<!--
			<div class="input-field col s6"  >
				<input id="idtipoalmacen" type="hidden" value=" " name="idtipoalmacen" > 
				<input id="tipoalmacen" type="number" value="" name="tipoalmacen" disabled readonly >
                <label class="active" for="pase">Pase</label> 
			</div>
			-->
			...
			<div class="input-field col s6"  >
				<input id="idtipoalmacen" type="hidden" value="" name="idtipoalmacen" >
				<input id="tipoalmacen" type="text" value="" name="tipoalmacen" autocomplete="off" disabled readonly > 
                <label  class="active" for="tipoalmacen" style="color:#0000006b" id="labelalmacen" >Almacen</label> 
			</div>
			
			
			<div class="input-field col s6"  >
				<input id="idtiposervicio" type="hidden" value=" " name="idtiposervicio" > 
				<input id="tiposervicio" type="text" value="" name="tiposervicio" autocomplete="off" disabled readonly > <!-- disabled readonly -->
                <label for="tiposervicio" style="color:#0000006b" id="labelservicio">Servicio</label> 
			</div>
	...
	function buscar()
    {
		...
		.then(function(data){
			if(data.length > 0 ){
			   ...
			   $("#pase").prop("disabled", false);
			   $("#pase").prop("readonly", false);
			   $("#temperatura").prop("disabled", false);
			   $("#temperatura").prop("readonly", false);
			   $("#labelempresa").css("color", "#000000");
			   $("#labelalmacen").css("color", "#000000");
			   $("#labelservicio").css("color", "#000000");
			   //console.log("1");
			   $("#autocomplete-input2").prop("readonly", false);
               //console.log("2");
			   $("#autocomplete-input2").prop("disabled", false);
			   $("#tipoalmacen").prop("disabled", false);
			   $("#tipoalmacen").prop("readonly", false);
			   $("#tiposervicio").prop("disabled", false);
			   $("#tiposervicio").prop("readonly", false);
...
function guardar()
{
  		...
  		console.log("pase:" + document.getElementById('pase').value);
  		console.log("temperatura:" + document.getElementById('temperatura').value);
  		console.log("idtipoalmacen:" + document.getElementById('idtipoalmacen').value);
  		console.log("idtiposervicio:" + document.getElementById('idtiposervicio').value);
	
	if(document.getElementById('pase').value.trim()  != '' &&
       document.getElementById('temperatura').value.trim() != '' &&
       document.getElementById('idtipoalmacen').value.trim() != '' &&
       document.getElementById('idtiposervicio').value.trim() != ''
      )
	{	
		let url = '<?= base_url() ?>api/execsp';
		let sp = "MOVIMIENTO_PERSONA_INS_AQPCALLAO";
		let empresa = <?= $empresa->EMPRES_N_ID ?>;
		let cliente_visitante = parseInt(document.getElementById("cliente_").value);
		...
		let tipoalmacen = document.getElementById("idtipoalmacen").value;
		let tiposervicio = document.getElementById("idtiposervicio").value;
   ...
   {
            M.toast({html: 'Debe llenar todos los campos PASE / TEMPERATURA/ALMACEN / SERVICIO', classes: 'rounded'});
   }	
	...
	function validar()
    {
        ...
        if( document.getElementById('tdocumento').value.trim()  != '' &&
                    ...
                    if(data.length>0){
					   ...
					   //nuevo
						...
						if(data.length>0){
							...
							$("#temperatura").attr("disabled",false);
							$("#labelempresa").css("color", "#000000");
			   				$("#labelalmacen").css("color", "#000000");
			   				$("#labelservicio").css("color", "#000000");
							$("#autocomplete-input2").prop("readonly", false);
							$("#autocomplete-input2").prop("disabled", false);
							$("#tipoalmacen").attr("readonly",false);
							$("#tipoalmacen").attr("disabled",false);
							$("#tiposervicio").attr("readonly",false);
...
function TipoAlmacen(){
	...
	fetch(url, {
		...
		.then(function(data){
			...
			tipoalmacenList[varray[i].TIPLALMACEN_C_DESCRIPCION] = null;
			}
			
			$("#tipoalmacen").autocomplete({
			data: tipoalmacenList,
			onAutocomplete: function(val) {
  			console.log(val);
			//let val2="motivo_visita";
			let val2="tipo_almacen";
			IdRetorno(val,val2);
...
function TipoServicio(){
	...
	fetch(url, {
		...
		.then(function(data){
			...
			var varray = data;
			var tiposervicioList = {};
			for (var i = 0; i < varray.length; i++) {
			tiposervicioList[varray[i].TIPSERVICIO_C_DESCRIPCION] = null;
			}
			$("#tiposervicio").autocomplete({
			data: tiposervicioList,
			onAutocomplete: function(val) {

  			console.log(val);
			let val2="tipo_servicio";
			IdRetorno(val,val2);
...
function ListadoCLienteVisitante(){
	...
	let url = '<?= base_url() ?>api/execsp';
	let sp = "LISTADO_ENTIDADES_AQPCALLAO";
	let idempresa = <?= $empresa->EMPRES_N_ID ?>;
	//let idcliente = 0;
...
function IdRetorno(val,tablanombre){
		...
		let data = {sp, txtval,tablanombre,idempresa};
		...
		.then(function(data){
			...
			else if(tablanombre=="tipo_almacen"){
				$("#idtipoalmacen" ).val(id);
			}else if(tablanombre=="tipo_servicio"){
				$("#idtiposervicio").val(id);
			}
